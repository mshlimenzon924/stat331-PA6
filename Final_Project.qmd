...
```{r}
Model <- lm(emissions ~ average_income_per_capita, data = merged_data)
summary(Model)
```
...
coef <- coef(Model)
print(paste("The Regression Equation is: Emissions =", as.character(coef[1]), "+", as.character(coef[2]), "*average_income_per_capita"))
```

Interpretation of the Intercept: For countries with an average income per capita of 0 constant international dollars, the estimated CO2 emissions per capita is .958. *do we have data points at 0? if not, this is not interpretable*

Interpretation of average_income_per_capita: Each increase in one constant international dollar is associated with an increase of .279 in CO2 emissions per capita for all countries similar to those in the study.
